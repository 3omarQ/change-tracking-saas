...
export function DatapointEditSection({ datapoint, invalidateKeys }: DatapointEditSectionProps) {
  ...
  const form = useForm<CreateJobFormData>({
    ...
    defaultValues: {
      ...
      fieldNames: datapoint.fieldNames
        ? JSON.parse(datapoint.fieldNames as string)
        : [],
    },
  });
    ...
    <>
      <JobDetailSection icon={DatabaseIcon} label="Datapoint" onEdit={() => setOpen(true)}>
        ...
        <JobDetailField
          label="CSS / HTML path"
          value={
            <code className="text-xs bg-muted px-1.5 py-0.5 rounded font-mono truncate max-w-2xs block">
              {datapoint.path}
            </code>
          }
        />
        {datapoint.fieldNames && (
          <JobDetailField
            label="Fields"
            value={
              <div className="flex flex-wrap gap-1 mt-0.5">
                {(JSON.parse(datapoint.fieldNames as string) as string[]).map((f, i) => (
                  <span
                    key={i}
